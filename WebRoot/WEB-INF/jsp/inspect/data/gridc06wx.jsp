...
$(function() {
	//初始化表格
	if(total<=page){
		$("#nextdown").attr('disabled',true);
	}
	list();
/*	$("#nextup").hover(function(){
		   $("#nextup").addClass("checked");
	},function(){
		   $("#nextup").removeClass("checked");
	});
	   $("#nextdown").hover(function(){
		   $("#nextdown").addClass("checked");
	},function(){
		   $("#nextdown").removeClass("checked");
	}
	);*/
});
...
function next(v){
	...
	if(page <= 1){
		page = 1;
		$("#nextup").attr('disabled',true);
	}else{
		$("#nextup").attr('disabled',false);
	}
	if(page>=total){
		page = total;
		$("#nextdown").attr('disabled',true);
	}else{
		$("#nextdown").attr('disabled',false);
	}
	 list();
}
